...
class GreaterOperator : Operator("greater", ">", OperatorType.INFIX) {
    override fun emit(instructions: InstructionsSet, environment: Environment, operatorExpression: OperatorExpression) {
        Operators.produceCompare(instructions, environment, operatorExpression, ComparisonOperation.GREATER)
    }
