...
public class ImportStatementParsingFunction extends AbstractParsingFunction<ImportStatement> {
    ...
    public ImportStatement parse(ParsingContext context, @Nullable Object... extra) {
        ...
        parser.next(TokenTypes.IMPORT(), Text.translatable("meazy:parser.expected.keyword", "import"));

        StringBuilder name = new StringBuilder(parser.getCurrentAndNext(
                TokenTypes.ID(), Text.translatable("meazy:parser.expected.after_keyword", "id", "import")).getValue()
        );

        while (parser.getCurrent().getType().equals(TokenTypes.DOT())) {
            parser.next();
            name.append(".");
            name.append(parser.getCurrentAndNext(TokenTypes.ID(), Text.translatable("meazy:parser.expected", "id")).getValue());
        }
